 ...
 */
export function setCustomProperties<T extends FabricObject>(
  obj: any,
 ...
 * });
 */
export function applyImageOptions(img: FabricImage, imageData: ImageToAdd): FabricImage {
  // Apply standard Fabric.js properties (excluding positioning and custom properties)
  if (!isNullOrUndefined(imageData.opacity)) img.set('opacity', imageData.opacity);
  if (!isNullOrUndefined(imageData.width)) img.set('width', imageData.width);
  if (!isNullOrUndefined(imageData.height)) img.set('height', imageData.height);
  if (!isNullOrUndefined(imageData.fill)) img.set('fill', imageData.fill);
  if (!isNullOrUndefined(imageData.stroke)) img.set('stroke', imageData.stroke);
  if (!isNullOrUndefined(imageData.strokeWidth)) img.set('strokeWidth', imageData.strokeWidth);
  if (!isNullOrUndefined(imageData.selectable)) img.set('selectable', imageData.selectable);
  if (!isNullOrUndefined(imageData.evented)) img.set('evented', imageData.evented);
  if (!isNullOrUndefined(imageData.scaleX)) img.set('scaleX', imageData.scaleX);
  if (!isNullOrUndefined(imageData.scaleY)) img.set('scaleY', imageData.scaleY);
  if (!isNullOrUndefined(imageData.angle)) img.set('angle', imageData.angle);
