...
export class ImageFactory {
   ...
   */
  async addFromUrl(imageData: ImageToAdd, select: boolean = true): Promise<FabricImage> {
    ...
    if (select) this.canvas.setActiveObject(img);
   ...
   */
  flip(image: FabricImage, direction: 'horizontal' | 'vertical' = 'horizontal'): void {
   ...
   */
  async crop(image: FabricImage): Promise<FabricImage> {
   ...
   * @returns Promise resolving to created image object
   */
  async addFromBase64(base64: string, options: ImageOptions = {}): Promise<FabricImage> {
   ...
   */
  async addMultipleFromUrl(
    images: ImageToAdd[],
    config: MultipleImagesConfig = {}
  ): Promise<FabricImage[]> {
    // Set defaults
    ...
    if (this.debug) {
      console.log(
        ...
        addedImages.length,
        'images',
        allHavePositions ? '(positioned)' : '(animated)'
      );
    }
