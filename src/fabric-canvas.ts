...
import { CanvasConfig, mergeConfig } from './config';
...
export class FabricCanvas {
  private canvas!: FabricCanvasType;
  private config: Required<CanvasConfig>;
  private canvasElement!: HTMLCanvasElement;
  private cleanupFunctions: (() => void)[] = [];

  // Factories
  public readonly shapes!: ShapeFactory;
  public readonly text!: TextFactory;
  public readonly images!: ImageFactory;
  ...
  public readonly serialization!: SerializationManager;

  // Utilities
  public readonly performance!: PerformanceUtility;
   ...
   * @param elementOrId - Canvas element or element ID
   */
  private initialize(elementOrId: string | HTMLCanvasElement): void {
    // Get canvas element
    if (typeof elementOrId === 'string') {
      const element = document.getElementById(elementOrId);
      if (!element || !(element instanceof HTMLCanvasElement)) {
        throw new Error(
          `Canvas element with id "${elementOrId}" not found or is not a canvas element`
        );
      }
    ...
    (this as any).shapes = new ShapeFactory(this.canvas, this.config.debug);
    (this as any).text = new TextFactory(this.canvas, this.config.debug);
    (this as any).images = new ImageFactory(this.canvas, this.config.debug);
    (this as any).selection = new SelectionManager(this.canvas, this.config.debug);
    (this as any).events = new EventManager(
      this.canvas,
      this.config.onChange.debounce,
      this.config.debug
    );
    (this as any).objects = new ObjectManager(this.canvas, this.config.debug, (this as any).events);
    (this as any).transform = new TransformManager(this.canvas, this.config.debug);
    (this as any).viewport = new ViewportManager(
      this.canvas,
      (this as any).events,
    ...
    (this as any).input = new InputManager(
      this.canvas,
      (this as any).selection,
    ...
    (this as any).history = new HistoryManager(this.canvas, (this as any).events, {
      maxSize: this.config.history.maxSize,
      debug: this.config.debug,
    });
    (this as any).serialization = new SerializationManager(this.canvas, this.config.debug);
    (this as any).performance = new PerformanceUtility(this.canvas, this.config.debug);

    // Connect InputManager undo/redo to HistoryManager
    (this as any).input.onUndo(() => this.history.undo());
    (this as any).input.onRedo(() => this.history.redo());
  ...
  configure(config: Partial<CanvasConfig>): void {
    ...
    if (config.preserveObjectStacking !== undefined) {
      (this.canvas as any).preserveObjectStacking = config.preserveObjectStacking;
    }
  ...
  fitToContainer(
    ...
    fitMode: 'height' | 'width' | 'auto' | 'contain' | 'cover' = 'auto',
    canvasWidth?: number,
    canvasHeight?: number
  ): void {
    ...
    (this.canvas as any).setViewportTransform([scale, 0, 0, scale, offsetX, offsetY]);
    this.canvas.requestRenderAll();
