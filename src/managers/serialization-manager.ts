...
export class SerializationManager {
  ...
  toJSON(propertiesToInclude?: string[]): CanvasJSON {
    ...
    const json = this.canvas.toObject(properties) as CanvasJSON;
   ...
   */
  async fromJSON(json: string | CanvasJSON, callback?: () => void): Promise<void> {
   ...
   */
  async toPNG(options: PNGExportOptions = {}): Promise<string> {
    ...
    const vpt: [number, number, number, number, number, number] = [1, 0, 0, 1, 0, 0];
   ...
   * @returns Promise resolving to data URL
   */
  async toJPEG(options: JPEGExportOptions = {}): Promise<string> {
    // Clone canvas to avoid blocking UI
    const clonedCanvas = await this.canvas.clone(['data']);
   ...
   */
  async toPDF(options: PDFExportOptions = {}): Promise<Blob> {
    ...
    const pdf = new jsPDF(
      ...
      options.unit ?? 'px',
      options.format ?? [width as number, height as number],
      false
    );
    ...
    // Add image to PDF
    pdf.addImage(dataURL, 'PNG', 0, 0, width as number, height as number);
