...
export class InputManager {
    constructor(canvas, selectionManager, viewportManager, objectManager, config = {}) {
        // Keyboard state
        ...
        this.touchPanLastX = 0;
        this.touchPanLastY = 0;
    ...
    enableSpacebarPan() {
        ...
        // Spacebar keydown
        this.spacebarKeydownListener = (e) => {
        ...
        this.mouseMoveListener = (opt) => {
            if (!this.spacebarPressed || !this.mouseDown)
                return;
            const e = opt.e;
    ...
    enableTouchPan() {
        ...
        this.hammer.on('panmove', (e) => {
            ...
            if (Math.abs(xChange) <= this.config.touchPanMaxJump &&
                Math.abs(yChange) <= this.config.touchPanMaxJump) {
                const delta = { x: xChange, y: yChange };
    ...
    handleCopy(event) {
        ...
        const unlocked = activeObjects.filter((obj) => !obj?.locked);
    ...
    handlePaste(event) {
        ...
        Promise.all(this.copiedObjects.map(async (obj) => {
